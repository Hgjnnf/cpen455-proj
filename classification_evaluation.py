...
def get_label(model, model_input, device):
    ...
    for c in range(NUM_CLASSES):
        candidate_labels = torch.full((batch_size,), c, dtype=torch.long, device=device)
        output = model(model_input, candidate_labels, sample=False)
        # Compute per-sample loss by processing each sample individually.
        for i in range(batch_size):
            x_i = model_input[i:i+1]
            l_i = output[i:i+1]
            loss_i = discretized_mix_logistic_loss(x_i, l_i)  # should return a scalar (B=1)
            losses[i, c] = loss_i
    predicted = torch.argmin(losses, dim=1)
    return predicted
# End of your code
...
if __name__ == '__main__':
    ...
    model = ConditionalPixelCNN(num_classes=NUM_CLASSES, embedding_dim=64,
